...
def get_profile() -> dict:
    """获取我的账号人设（垂类/语气/禁忌词/标签池等）"""
    from app.db.connection import get_db
    conn = get_db()
    try:
        row = conn.execute("SELECT * FROM my_profile WHERE id=1").fetchone()
        if not row:
            return {"error": "尚未初始化账号人设，请先运行 CLI: profile init"}
        d = dict(row)
...
def update_profile(
    ...
    try:
        ...
        params.append(1)
        conn.execute(
            f"UPDATE my_profile SET {', '.join(set_clauses)} WHERE id=?", params
        )
        conn.commit()
    finally:
        conn.close()
...
def list_items(tag: str = None, style: str = None) -> list[dict]:
    """列出图库物品。可按 tag（标签）或 style（风格）过滤"""
    from app.modules.library.manager import list_items as _list
    items = _list(tag=tag, style=style)
    return [i.model_dump() for i in items]


@mcp.tool()
def get_item(item_id: int) -> dict:
    """获取图库物品详情，包含 MiniMax 图片分析结果"""
...
def add_item_from_path(image_path: str, title: str = None, analyze: bool = True) -> dict:
    """从本地路径导入图片到图库，analyze=True 时自动触发 MiniMax 图片分析"""
    ...
    item = _add(image_path, title=title, analysis=analysis)
    return item.model_dump()


@mcp.tool()
def tag_item(item_id: int, tag: str, action: str = "add") -> dict:
    """给图库物品添加或删除标签。action: 'add' 或 'remove'"""
    from app.modules.library.manager import add_tag, remove_tag
    if action == "add":
        item = add_tag(item_id, tag)
    else:
        item = remove_tag(item_id, tag)
    return item.model_dump()
...
def draft_note_prompt(item_id: int, account_id: str = None, save: bool = True) -> dict:
    ...
    note_id = None
    if save:
        note = create_note(item_id=item_id, account_ref=account_id, prompt_used=prompt)
        note_id = note.id
...
def list_notes(status: str = None, item_id: int = None) -> list[dict]:
    """列出笔记草稿。status: draft/ready/published；item_id: 按物品过滤"""
    from app.modules.content.manager import list_notes as _list
    notes = _list(status=status, item_id=item_id)
