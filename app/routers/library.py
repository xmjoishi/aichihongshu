...
def api_list_items(
    ...
    return [_with_abs_path(i) for i in list_items(tag=tag, style=style, offset=offset, limit=limit, account_pool_id=pool_id)]
...
async def api_add_item(
    ...
    try:
        # 先入库（不分析），立即返回
        item = add_item(tmp_path, title=title, analysis=None, account_pool_id=_active_pool_id())
    finally:
        os.unlink(tmp_path)
...
def api_tag_item(item_id: int, body: TagAction):
    ...
    if not item:
        raise HTTPException(404, f"物品 {item_id} 不存在")
    if body.action == "add":
        return add_tag(item_id, body.tag, account_pool_id=pool_id)
    elif body.action == "remove":
        return remove_tag(item_id, body.tag, account_pool_id=pool_id)
    raise HTTPException(400, "action 必须是 add 或 remove")


@router.delete("/{item_id}")
def api_delete_item(item_id: int, delete_file: bool = Query(False)):
    ok = delete_item(item_id, account_pool_id=_active_pool_id())
    if not ok:
        raise HTTPException(404, f"物品 {item_id} 不存在")
    return {"ok": True}
...
def api_list_trash():
    """查询回收站中的物品"""
    return [_with_abs_path(i) for i in list_trash(account_pool_id=_active_pool_id())]


@router.post("/trash/{item_id}/restore")
def api_restore_item(item_id: int):
    """从回收站恢复物品"""
    ok = restore_item(item_id, account_pool_id=_active_pool_id())
    if not ok:
...
def api_purge_item(item_id: int):
    """物理删除单个回收站物品"""
    ok = purge_item(item_id, account_pool_id=_active_pool_id())
    if not ok:
        raise HTTPException(404, f"物品 {item_id} 不在回收站中")
    return {"ok": True}
...
def api_purge_all():
    """清空回收站（物理删除所有）"""
    count = purge_all_trash(account_pool_id=_active_pool_id())
